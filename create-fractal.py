...
from fractal.polymandel import polymandel
...
def config(argv=sys.argv[1:]):
    ...
    parser.add_argument('-k', '--kws', type=str, default="",
                        help=("Pass optional keywords to complex function "
                              """as "arg1=3,arg2=8.5,arg3='spam'" """))
    return parser.parse_args(argv)


def generate(args):
    data = make_canvas(args.fn, args.xcenter, args.ycenter,
                           args.size, args.pixels, kws=args.kws)
...
def main(args):
    fname = f"{args.kind}_{args.xcenter}_{args.ycenter}_"\
            f"{args.size}_{args.raw_kws}.png"
    im = generate(args)
    im.save(fname)
...
if __name__ == '__main__':
    args = config()
    args.raw_kws = args.kws
    args.kws = eval(f"dict({args.kws})")
